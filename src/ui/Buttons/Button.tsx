...
function Button({ variant = "default", icons, children, onClick, className }: { variant?: "default" | "blue", icons?: React.ReactElement, children: React.ReactNode, onClick?: () => void, className?: string }) {
    ...
    return (
        <button onClick={onClick && onClick} className={cn(`${variants[variant]} p-2 rounded-md xss:text-[11px] md:text-xs flex items-center gap-2 w-28 justify-center transition-all duration-300`, className)}>{icons} {children}</button>
    )
}

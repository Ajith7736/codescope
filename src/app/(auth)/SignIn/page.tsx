...
import { singInInputs, singInSchema } from '@/lib/client/Schemas/singinschema/Schema'
    ...
    return (
        <div className={`relative w-screen min-h-dvh flex flex-col gap-3 justify-center items-center p-8`}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className={`flex flex-col ${(errors.email || errors.password) ? 'gap-2' : 'gap-3'} xss:w-[20rem] w-[24rem] items-center text-sm`}>
                <InputBox errors={errors.email?.message} id='email' placeholder='Enter your email' register={register} type='email' />
                <InputBox errors={errors.password?.message} id='password' placeholder='Enter your password' register={register} type='password' />

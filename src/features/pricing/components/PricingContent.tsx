            ...
            <div className="flex flex-col items-center gap-10 px-5">
                ...
                <div className="flex xss:flex-col md:flex-row md:flex-wrap w-full md:h-[60vh] md:items-center md:justify-center gap-8">
                    {plans?.map((item, index) => {
                        return <div key={index} className={`${currentplan === item.name && session ? 'dark:bg-white/50 bg-black/30' : 'dark:conic'} relative p-px rounded-xl xss:w-full md:w-[70%] lg:w-[30%]`}>
                            ...
                            <div className="dark:bg-black bg-gray-400/20 shadow-lg flex flex-col items-baseline gap-5 w-full h-[60vh] justify-between relative p-5 overflow-hidden rounded-xl">
                                ...
                                {session ? <button onClick={() => handlerazorpay(item.razorpayPlanId)} className={`rounded-md w-full font-extrabold transition-all duration-300 dark:bg-white bg-black text-white dark:text-black p-2 hover:dark:bg-white/90 hover:bg-black/80  cursor-pointer ${item.name === "Free Tier" && 'invisible'}`}>Subscribe</button> : <button onClick={() => { router.push("/Signup") }} className="rounded-md w-full font-extrabold transition-all duration-300 dark:bg-white bg-black text-white dark:text-black p-2 hover:dark:bg-white/90 hover:bg-black/80  cursor-pointer">Get Started</button>}
                            </div>
                        </div>
                    })}
                </div>

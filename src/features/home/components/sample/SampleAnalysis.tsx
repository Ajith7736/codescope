...
function sample({ showcode, setshowcode }: { showcode: showcodeprops, setshowcode: React.Dispatch<React.SetStateAction<showcodeprops>> }) {
  ...
  `,
    ...
    return (
        <div className='bg-light-surface dark:bg-dark-border/20  rounded-xs dark:border-dark-border xss:w-[20rem] sm:w-md p-5 border border-light-border flex flex-col gap-3'>
            {samples.map((sample) => {

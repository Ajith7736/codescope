...
function SettingsPage({ projectId, refetch }: { projectId: string | undefined, refetch: Function }) {
  ...
  return (
    <div className='p-5 border m-5  md:w-[90%] border-red-500/30 bg-dark-surface/30 rounded-md flex flex-col gap-5'>
      ...
      {loading ? <button className="text-sm text-red-500 font-bold cursor-pointer bg-red-500/20 h-12 rounded-xs hover:bg-red-500/30 transition-all duration-300"><ButtonLoader /></button> : <button onClick={handleproject} className="text-xs text-red-500 font-bold cursor-pointer bg-red-500/20 h-12 rounded-xs hover:bg-red-500/30 transition-all duration-300">Delete this repositary</button>}
    </div>
  )
}

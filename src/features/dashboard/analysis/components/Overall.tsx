...
function Overall({item} : {item : AnalysisProps}) {
    return (
        <div className='flex flex-col gap-2 w-fit items-center'>
            <div className={`bg-dark-hovergray ${item.type === "Architecture" ? 'text-blue-500' : item.type === "Security" ?  'text-red-500' : 'text-green-500'} border border-light-gray/10 p-5 rounded-md  backdrop-blur-2xl`}>
                {item.icon}
            </div>
